sap.ui.define([
	...
	return Controller.extend("helpe.fastentrysol.controller.BaseController", {
		/**
		...
		_onValueHelpRequest: function (oInput) {
			if (oInput !== "Close" && oInput !== "Destroy") {
				...
				this._oValueHelpDialog = sap.ui.xmlfragment("helpe.fastentrysol.fragment.ValueHelpDialog", this);
		...
		onLicenseVHFilter: function (oEvent,that) {
			...
			var oSalesModel = that.getView().getModel("SalesOrder")
			var matnr = oSalesModel.getData().to_OrdersChangeItems.results[that.selectedrow].matnr;
			var matnrFilter1 = new sap.ui.model.Filter("matnr", "EQ", matnr);
			oBindingParams.filters.push(matnrFilter1);

			var CompCode = oSalesModel.getProperty("/bukrs") || "G001";
			var CompCodeFilter2 = new sap.ui.model.Filter("bukrs_vf", "EQ", CompCode);
			oBindingParams.filters.push(CompCodeFilter2);

			var customer = oSalesModel.getProperty("/kunnr")
			var customerFilter3 = new sap.ui.model.Filter("SoldToParty", "EQ", customer);
			oBindingParams.filters.push(customerFilter3);

			var Plant = oSalesModel.getProperty("/werks")
